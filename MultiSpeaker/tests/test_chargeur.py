...
# -- Tests Matcha-Conformer --

def _create_fake_matcha_models(directory: Path, speaker: str = "siwis"):
    """Cree un jeu de modeles factices (Matcha per-speaker) dans un repertoire."""
    directory.mkdir(parents=True, exist_ok=True)
    (directory / "matcha_unet.onnx").write_bytes(b"FAKE")
    (directory / "hifigan.onnx").write_bytes(b"FAKE")
    (directory / f"matcha_encoder_{speaker}.onnx").write_bytes(b"FAKE")
    (directory / "config.json").write_text("{}")
    (directory / "phoneme_vocab.json").write_text('{"vocab":[],"phone2id":{}}')


def test_has_models_matcha(tmp_path):
    """Detecte les modeles Matcha-Conformer."""
    from lectura_tts_multispeaker._chargeur import _has_models

    _create_fake_matcha_models(tmp_path)
    assert _has_models(tmp_path, "siwis") is True
    assert _has_models(tmp_path, "bernard") is False


def test_has_models_matcha_multi_speakers(tmp_path):
    """Detecte plusieurs speakers Matcha."""
    from lectura_tts_multispeaker._chargeur import _has_models

    _create_fake_matcha_models(tmp_path, speaker="siwis")
    (tmp_path / "matcha_encoder_bernard.onnx").write_bytes(b"FAKE")

    assert _has_models(tmp_path, "siwis") is True
    assert _has_models(tmp_path, "bernard") is True


def test_find_models_dir_matcha(tmp_path):
    """Trouve le repertoire avec des modeles Matcha."""
    from lectura_tts_multispeaker._chargeur import find_models_dir
...
def test_has_models_matcha_missing_unet(tmp_path):
    """Matcha sans matcha_unet.onnx ne suffit pas (mais FastPitch oui)."""
    from lectura_tts_multispeaker._chargeur import _has_models

    tmp_path.mkdir(parents=True, exist_ok=True)
    (tmp_path / "hifigan.onnx").write_bytes(b"FAKE")
    (tmp_path / "matcha_encoder_siwis.onnx").write_bytes(b"FAKE")
    # Pas de matcha_unet.onnx ni decoder.onnx
    assert _has_models(tmp_path, "siwis") is False
