#!/usr/bin/env python3
"""Entraîne le modèle unifié P2G+POS+Morpho.

Phase 1 : Pré-entraînement P2G sur le lexique (mots IPA isolés)
...
from lectura_p2g.modele import (
    UnifiedP2G,
    LexiqueP2GDataset,
    UnifiedP2GDataset,
    MultiTaskLoss,
...
def eval_p2g_lexique(
    model: UnifiedP2G,
    data: list[dict],
    char2idx: dict[str, int],
    idx2p2g: dict[int, str],
    device: torch.device,
    max_items: int = 5000,
) -> dict[str, float]:
    """Évalue le P2G sur des mots isolés."""
    model.eval()
    ...
    with torch.no_grad():
        for item in items:
            ...
            char_ids = torch.tensor(
                [[char2idx.get(ch, 1) for ch in ipa]],
                dtype=torch.long,
            ).to(device)

            outputs = model(char_ids)
            pred_ids = outputs["p2g_logits"][0].argmax(dim=-1).cpu().tolist()
            pred_labels = [idx2p2g.get(idx, _CONT) for idx in pred_ids[:len(ipa)]]
            ...
            total_chars += len(gold_chars)
            edit_distance_sum += _levenshtein(pred_chars, gold_chars)

    word_acc = correct_words / total_words if total_words else 0
    cer = edit_distance_sum / total_chars if total_chars else 0

    return {"word_acc": word_acc, "cer": cer, "n_words": total_words}
...
def train_phase1(
    ...
    """Phase 1 : Pré-entraînement P2G sur le lexique."""
    print("\n" + "=" * 60)
    print("PHASE 1 : Pré-entraînement P2G (lexique)")
    print("=" * 60)

    char2idx = vocabs["char2idx"]
    p2g_label2idx = vocabs["p2g_label2idx"]
    idx2p2g = {v: k for k, v in p2g_label2idx.items()}

    dataset = LexiqueP2GDataset(train_data, char2idx, p2g_label2idx)
    loader = DataLoader(
    ...
    for name, param in model.named_parameters():
        if "word_lstm" in name or "pos_head" in name or "morpho" in name:
    ...
    n_labels = len(p2g_label2idx)

    print(f"  Dataset : {len(dataset)} mots")
    print(f"  Params P2G entraînés : "
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1 or epoch == epochs:
            ...
            if eval_data:
                metrics = eval_p2g_lexique(model, eval_data, char2idx, idx2p2g, device)
                msg += f" | word_acc={metrics['word_acc']:.1%} CER={metrics['cer']:.1%}"
...
def train_phase2(
    ...
    t0 = time.time()
    train_dataset = UnifiedP2GDataset(train_sentences, vocabs, phone_to_graphs)
    print(f"  Train dataset : {len(train_dataset)} phrases ({time.time() - t0:.0f}s)")

    dev_dataset = None
    if dev_sentences:
        print("  Préparation dataset dev...")
        dev_dataset = UnifiedP2GDataset(dev_sentences, vocabs, phone_to_graphs)
    ...
    criterion = MultiTaskLoss(
        w_p2g=1.0, w_pos=1.0, w_morpho=0.5,
    ...
    print(f"  Loss weights : p2g=1.0 pos=1.0 morpho=0.5")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_data in train_loader:
            ...
            targets = {
                "p2g_ids": batch_data["p2g_ids"].to(device),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Entraîne le modèle unifié P2G+POS+Morpho"
    )
    parser.add_argument(
        "--donnees", type=Path,
    ...
    parser.add_argument("--word-feedback", action="store_true",
                        help="Enable word→char feedback for P2G head (v2)")
    ...
    lexique_train = []
    lex_path = donnees / "lexique_p2g_train.json"
    if lex_path.exists():
        with open(lex_path, encoding="utf-8") as f:
            lexique_train = json.load(f)

    lexique_eval = []
    lex_eval_path = donnees / "lexique_p2g_eval.json"
    ...
    print(f"  Vocabs chars: {len(vocabs['char2idx'])}, "
          f"p2g: {len(vocabs['p2g_label2idx'])}, "
    ...
    model = UnifiedP2G(
        ...
        n_p2g_labels=len(vocabs["p2g_label2idx"]),
        ...
        word_feedback=args.word_feedback,
    ).to(device)
    ...
    # ── Phase 2 ──
    model_name = "unifie_p2g_v2.pt" if args.word_feedback else "unifie_p2g.pt"
    save_path = args.sortie / model_name
    train_phase2(
