"""Inférence NumPy (sans PyTorch/ONNX) pour le modèle unifié P2G V2 (avec lex_features).
...
class NumpyInferenceEngine:
    """Inférence NumPy pour le modèle unifié P2G V2."""
    ...
    def analyser(
        ...
        *, use_lex: bool = True,
        ...
        # Lex features
        if use_lex and self.has_lex_proj and ortho_words:
            lex_feats = np.array(
                [_build_lex_features(ortho_words[w] if w < len(ortho_words) else "", self.lexicon)
                 for w in range(n_words)],
                dtype=np.float32,
            )
