#!/usr/bin/env python3
"""Évaluation du POS Tagger Lectura (BiLSTM) sur les corpus Universal Dependencies.

Compare deux configurations :
  1. BiLSTM seul (sans mini-lexique)
  2. BiLSTM + mini-lexique (post-traitement)

Usage :
    python evaluer.py --test chemin/vers/pos_test_merged.conllu
    python evaluer.py --test ../POS_CRF/entrainement/donnees/pos_test_merged.conllu
"""
...
def print_comparison(r1: dict, r2: dict) -> None:
    ...
    print(f"\n{'=' * 72}")
    print(f"  COMPARAISON : BiLSTM seul vs BiLSTM + mini-lexique")
    print(f"{'=' * 72}\n")

    diff = r2["correct"] - r1["correct"]
    print(f"  {'Métrique':<25} {'BiLSTM seul':>12} {'BiLSTM + lex':>14} {'Diff':>8}")
    print(f"  {'-' * 62}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Évaluation du POS Tagger BiLSTM Lectura")
    parser.add_argument(
        "--test", required=True,
        help="Fichier CoNLL-U de test (ex: pos_test_merged.conllu)",
    )
    parser.add_argument(
        "--model", default=str(here / "modele" / "pos_model_bilstm_int8.onnx"),
        help="Chemin du modèle ONNX",
    )
    parser.add_argument(
        "--vocab", default=str(here / "modele" / "pos_vocab_bilstm.json"),
        help="Chemin du vocabulaire",
    )
    parser.add_argument(
    ...
    # ── Config 1 : BiLSTM seul ──
    print("\nChargement du modèle BiLSTM...")
    tagger_base = PosTagger(args.model, vocab_path=args.vocab)

    print("Évaluation BiLSTM seul...")
    pred_base = [
        [tag for _, tag in tagger_base.tag_words(words)]
        for words in words_all
    ]
    results_base = evaluate(pred_base, gold_all)
    print_results("BiLSTM seul (sans mini-lexique)", results_base)

    # ── Config 2 : BiLSTM + mini-lexique ──
    lexicon_path = Path(args.lexicon)
    if lexicon_path.exists():
        print("\nChargement du mini-lexique...")
        tagger_lex = PosTagger(args.model, vocab_path=args.vocab, lexicon_path=args.lexicon)
        n_entries = len(tagger_lex.lexicon)
        print(f"  {n_entries} entrées chargées")

        print("Évaluation BiLSTM + mini-lexique...")
        pred_lex = [
            [tag for _, tag in tagger_lex.tag_words(words)]
            for words in words_all
        ]
        results_lex = evaluate(pred_lex, gold_all)
        print_results("BiLSTM + mini-lexique", results_lex)

        # Comparaison
        print_comparison(results_base, results_lex)

        # Export de la meilleure config
        export_results(results_lex, "BiLSTM + mini-lexique", Path(args.export))
    else:
        print(f"\n  (mini-lexique non trouvé : {lexicon_path}, évaluation simple)")
        export_results(results_base, "BiLSTM seul", Path(args.export))
