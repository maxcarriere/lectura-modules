#!/usr/bin/env python3
"""Entraine un modele CRF G2P et l'exporte en JSON.

Architecture : features caractere → CRF → export JSON → Viterbi.

Pre-requis :
    pip install sklearn-crfsuite

Usage :
    python entrainer_crf.py --train train_g2p.csv --eval eval_g2p.csv
    python entrainer_crf.py --train train_g2p.csv --output modele.json
"""
...
from lectura_g2p import _extract_char_features

_CONT = "_CONT"


def load_dataset(path: Path) -> list[tuple[str, list[str]]]:
    """Charge un CSV (ortho, phone, aligned_labels) → [(word, labels)]."""
...
def extract_features(word: str) -> list[dict]:
    """Extrait les features pour un mot."""
    return [_extract_char_features(word, i) for i in range(len(word))]


def train_crf(train_data, c1: float = 0.1, c2: float = 0.1, max_iter: int = 100):
    """Entraine un modele CRF."""
    try:
    ...
    for word, labels in train_data:
        ...
        # Convertir les features en format CRF (str values)
        str_feats = []
        for f in feats:
            str_feats.append({k: str(v) for k, v in f.items()})
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Entrainement CRF G2P")
    ...
    parser.add_argument("--output", default="../modele/g2p_model_crf.json",
                        help="Fichier JSON de sortie")
    parser.add_argument("--c1", type=float, default=0.1, help="Regularisation L1")
    parser.add_argument("--c2", type=float, default=0.1, help="Regularisation L2")
    ...
    crf = train_crf(train_data, c1=args.c1, c2=args.c2, max_iter=args.max_iter)
    ...
    if args.eval:
        ...
        if eval_path.exists():
            ...
            correct = 0
            total = 0
            for word, gold_labels in eval_data:
                feats = extract_features(word)
                str_feats = [{k: str(v) for k, v in f.items()} for f in feats]
                pred_labels = crf.predict_single(str_feats)
                gold_ipa = "".join(l for l in gold_labels if l != _CONT)
                pred_ipa = "".join(l for l in pred_labels if l != _CONT)
                total += 1
                if pred_ipa == gold_ipa:
                    correct += 1
