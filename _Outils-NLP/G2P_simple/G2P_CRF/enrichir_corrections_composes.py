#!/usr/bin/env python3
"""Enrichir les corrections G2P CRF avec les parties de mots composés/apostrophe.
...
# On importe depuis le lectura_g2p.py local (G2P_CRF/)
...
# ═══════════════════════════════════════════════════════════════════════════════
# Étape 4 : Charger le modèle CRF
# ═══════════════════════════════════════════════════════════════════════════════

def load_model() -> LecturaG2P:
    """Charge le modèle CRF (sans corrections)."""
...
def build_corrections(
    ...
    """Teste le modèle CRF sur les parties orphelines et construit les corrections.
...
def update_correction_file(
    ...
    """Met à jour le fichier de corrections CRF.

    Returns:
        Nombre de corrections ajoutées.
    """
    here = Path(__file__).parent
    fpath = here / "modele" / "g2p_corrections_crf.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Enrichir les corrections G2P CRF avec les parties de mots composés.",
    )
    parser.add_argument(
        "--dico",
        type=Path,
    ...
    print("\n═══ Étape 3 : Chargement du modèle CRF ═══")
    model = load_model()
    ...
    print("\n═══ Étape 5 : Test du modèle CRF et construction des corrections ═══")
    corrections = build_corrections(orphans_gold, model)
    print(f"  CRF : {len(corrections)} corrections nécessaires")
    ...
    for check_word in ["lorsqu", "aujourd", "quelqu", "puisqu", "quoiqu", "jusq"]:
        if check_word in orphans_gold:
            ...
            print(f"  Vérification '{check_word}' → gold: /{gold}/ — CRF: /{pred}/ {status}{' → correction ajoutée' if needs_corr else ''}")
    ...
    print(f"  Gold déterminés           : {len(orphans_gold)}")
    print(f"  Corrections CRF           : {n_added}")
