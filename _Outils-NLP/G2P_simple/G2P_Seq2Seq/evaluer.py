#!/usr/bin/env python3
"""Evaluation du G2P Lectura (backend Seq2Seq) sur un fichier gold (dico.csv).
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluation du G2P Lectura (Seq2Seq)")
    parser.add_argument(
        "--gold", required=True,
    ...
    g2p_base = LecturaG2P(
        encoder_path, decoder_path=decoder_path, vocab_path=vocab_path,
    )
    ...
    if corr_path:
        ...
        g2p_corr = LecturaG2P(
            ...
            vocab_path=vocab_path, corrections_path=corr_path,
        )
