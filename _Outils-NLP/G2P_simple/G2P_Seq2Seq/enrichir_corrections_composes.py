#!/usr/bin/env python3
"""Enrichir les corrections G2P Seq2Seq avec les parties de mots composés/apostrophe.
...
# On importe depuis le lectura_g2p.py local (G2P_Seq2Seq/)
...
# ═══════════════════════════════════════════════════════════════════════════════
# Étape 4 : Charger le modèle Seq2Seq
# ═══════════════════════════════════════════════════════════════════════════════

def load_model() -> LecturaG2P:
    """Charge le modèle Seq2Seq (sans corrections)."""
    ...
    if not encoder_path.exists():
        print(f"ERREUR : modèle non trouvé : {encoder_path}", file=sys.stderr)
        sys.exit(1)

    return LecturaG2P(encoder_path, decoder_path=decoder_path, vocab_path=vocab_path)
...
def build_corrections(
    ...
    """Teste le modèle Seq2Seq sur les parties orphelines et construit les corrections.
...
def update_correction_file(
    ...
    """Met à jour le fichier de corrections Seq2Seq.

    Returns:
        Nombre de corrections ajoutées.
    """
    here = Path(__file__).parent
    fpath = here / "modele" / "g2p_corrections_seq2seq.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Enrichir les corrections G2P Seq2Seq avec les parties de mots composés.",
    )
    parser.add_argument(
        "--dico",
        type=Path,
    ...
    print("\n═══ Étape 3 : Chargement du modèle Seq2Seq ═══")
    model = load_model()
    ...
    print("\n═══ Étape 5 : Test du modèle Seq2Seq et construction des corrections ═══")
    corrections = build_corrections(orphans_gold, model)
    print(f"  Seq2Seq : {len(corrections)} corrections nécessaires")
    ...
    for check_word in ["lorsqu", "aujourd", "quelqu", "puisqu", "quoiqu", "jusq"]:
        if check_word in orphans_gold:
            ...
            print(f"  Vérification '{check_word}' → gold: /{gold}/ — Seq2Seq: /{pred}/ {status}{' → correction ajoutée' if needs_corr else ''}")
    ...
    print(f"  Gold déterminés           : {len(orphans_gold)}")
    print(f"  Corrections Seq2Seq       : {n_added}")
