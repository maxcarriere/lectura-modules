...
phoneme-par-phoneme pour l'entrainement BiLSTM P2G.
...
def _align_phonemes_to_ortho(phone_str: str, ortho: str) -> list[str] | None:
    """Aligne phonemes → graphemes (un label par phoneme).

    Retourne une liste de labels (un par phoneme), ou None si
    l'alignement echoue.
    """
...
def align_phone_ortho(phone_str: str, ortho: str) -> list[str] | None:
    """Aligne une transcription IPA avec un mot (P2G) — un label par char Unicode.

    Le BiLSTM opere caractere par caractere (embedding par char Unicode).
    Les combining marks IPA (ex: la tilde dans 'ɑ̃') recoivent le label _CONT.

    Retourne une liste de labels de meme longueur que ``phone_str``
    (un label par caractere Unicode), ou None si l'alignement echoue.
    """
    # 1. Aligner par phoneme d'abord
    phoneme_labels = _align_phonemes_to_ortho(phone_str, ortho)
    if phoneme_labels is None:
        return None

    # 2. Etendre les labels par phoneme en labels par char Unicode
    phonemes = iter_phonemes(phone_str)
    char_labels: list[str] = []
    for phoneme, label in zip(phonemes, phoneme_labels):
        # Premier char Unicode du phoneme recoit le label
        char_labels.append(label)
        # Les chars Unicode suivants (combining marks) recoivent _CONT
        for _ in range(1, len(phoneme)):
            char_labels.append(_CONT)

    # Verification : doit correspondre a len(phone_str) exactement
    if len(char_labels) != len(phone_str):
        return None
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Preparation dataset P2G (BiLSTM)")
