"""Lectura P2G — Convertisseur phoneme-grapheme pour le francais (backend CRF).

Fichier unique, autonome, zero dependance externe.
...
# ══════════════════════════════════════════════════════════════════════════════
# Features CRF (extraction de features par caractere IPA)
# ══════════════════════════════════════════════════════════════════════════════
...
def _extract_ipa_features(ipa_chars: list[str], idx: int) -> dict[str, str | float]:
    """Extrait les features pour le caractere IPA ipa_chars[idx].

    Adapte de G2P CRF mais avec features specifiques IPA :
    - is_vowel : voyelle IPA (orale)
    - is_nasal_vowel : voyelle nasale IPA
    - is_consonant : consonne IPA
    - is_semi_vowel : semi-voyelle IPA
    """
    n = len(ipa_chars)
    ch = ipa_chars[idx]
...
def _extract_ipa_word_features(ipa_chars: list[str]) -> list[dict[str, str | float]]:
    """Extrait les features pour tous les caracteres IPA."""
...
# ══════════════════════════════════════════════════════════════════════════════
# Modele CRF + Viterbi (+ beam Viterbi pour top-K)
# ══════════════════════════════════════════════════════════════════════════════
...
class CrfP2gModel:
    ...
    def _viterbi_topk(
        ...
        Garde K chemins a chaque pas au lieu de 1.
        """
        n = len(observations)
        if n == 0:
            return [([], 0.0)]
        ...
        # Convertir en sequences de tags
    ...
    def predict_candidates(
        ...
        Utilise le beam Viterbi pour generer K chemins.
        """
        if not ipa:
            return [("", 1.0)]
        ...
        # Normaliser par longueur et dedupliquer
        ...
        # Convertir log-scores en probabilites normalisees
        if results:
...
class LecturaP2G:
    """Convertisseur Phoneme → Grapheme pour le francais (backend CRF).

    Strategies (par ordre de priorite pour predict()) :
    1. Modele CRF (si model_path fourni)
    2. Table P2G (embarquee ou externe)
    3. Regles deterministes (fallback universel)

    Args:
        model_path: Chemin vers le modele CRF (fichier JSON).
    ...
    def __init__(
        ...
        self._model: CrfP2gModel | None = None
        self._table: dict[str, str] = {}
        self._backend = "crf"

        # Charger le modele CRF si disponible
        if model_path is not None:
            model_path = Path(model_path)
            if model_path.exists():
                self._model = CrfP2gModel.load(model_path)
    ...
    def has_model(self) -> bool:
        """True si un modele CRF est charge."""
        return self._model is not None

    def predict(self, ipa: str) -> str:
        """Meilleure orthographe pour une chaine IPA.

        Utilise le modele CRF si disponible, sinon table, sinon regles.
    ...
    def predict_candidates(
        ...
        Avec le modele CRF, utilise le beam Viterbi.
        Sans modele, retourne un seul candidat (table ou regles) avec prob 1.0.
...
def _main() -> None:
    ...
    p2g = LecturaP2G(model_path=model_path, table_path=table_path)
    mode = "CRF" if p2g.has_model else "table+regles"
    ...
    if interactive:
        ...
        print("Usage: python lectura_p2g.py [--model FILE] [--table FILE] [-k N] [-i] <ipa> ...")
        print("\nExemple :")
