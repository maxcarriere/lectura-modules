"""Lectura P2G — Convertisseur phoneme-grapheme pour le francais.

Module racine unifie supportant trois backends :
  - CRF : zero dependance, modele JSON (Viterbi + beam search)
  - BiLSTM : onnxruntime, modele ONNX (sequence-labeling + perturbation)
  - Seq2Seq : onnxruntime, encoder-decoder ONNX (beam search)

Auto-detection du backend par extension de fichier :
  .json          → CRF
  .onnx (seul)   → BiLSTM
  .onnx (paire)  → Seq2Seq (encoder + decoder)

Usage rapide :
    from lectura_p2g import LecturaP2G

    p2g = LecturaP2G("P2G_CRF/modele/p2g_model_crf.json")
    p2g.predict("bɔ̃ʒuʁ")                     # → "bonjour"
    p2g.predict_candidates("pɛʃœʁ", k=5)      # → [("pêcheur", 0.42), ...]
    p2g.predict_syllable("kɑ̃")                # → "quand"

Pre-requis :
  - Zero dependance pour CRF + table + regles (Python 3.10+)
  - onnxruntime + numpy pour BiLSTM et Seq2Seq
...
_IPA_NASAL_BASES: set[str] = {"ɑ", "ɛ", "ɔ", "œ"}

_IPA_CONSONANTS: set[str] = {
    "p", "b", "t", "d", "k", "ɡ", "g", "f", "v", "s", "z",
    "ʃ", "ʒ", "m", "n", "ɲ", "ŋ", "l", "ʁ",
}

_IPA_SEMI_VOWELS: set[str] = {"j", "w", "ɥ"}
...
def _is_ipa_vowel(phoneme: str) -> bool:
    ...
    if not phoneme:
        return False
    if phoneme in _IPA_VOWELS:
        return True
    return bool(phoneme[0] in _IPA_VOWELS)


def _is_nasal_vowel(phoneme: str) -> bool:
    """Vrai si le phoneme est une voyelle nasale IPA (ex: ɑ̃, ɛ̃)."""
    if len(phoneme) < 2:
        return False
    return phoneme[0] in _IPA_NASAL_BASES and "\u0303" in phoneme


def _is_ipa_consonant(phoneme: str) -> bool:
    """Vrai si le phoneme est une consonne IPA."""
    return bool(phoneme and phoneme in _IPA_CONSONANTS)
...
# ══════════════════════════════════════════════════════════════════════════════
# Features CRF (extraction depuis phonemes IPA)
# ══════════════════════════════════════════════════════════════════════════════

_CONT = "_CONT"


def _extract_ipa_features(ipa_chars: list[str], idx: int) -> dict[str, str | float]:
    """Extrait les features pour le phoneme ipa_chars[idx]."""
    n = len(ipa_chars)
    ch = ipa_chars[idx]
    ...
    feats: dict[str, str | float] = {
        "bias": 1.0,
        "char": ch,
        "is_vowel": 1.0 if _is_ipa_vowel(ch) else 0.0,
        "is_nasal_vowel": 1.0 if _is_nasal_vowel(ch) else 0.0,
        "is_consonant": 1.0 if _is_ipa_consonant(ch) else 0.0,
        "is_semi_vowel": 1.0 if ch in _IPA_SEMI_VOWELS else 0.0,
    ...
    feats["char-1"] = ipa_chars[idx - 1] if idx > 0 else "__BOS__"
    feats["char-2"] = ipa_chars[idx - 2] if idx > 1 else "__BOS__"
    feats["char+1"] = ipa_chars[idx + 1] if idx < n - 1 else "__EOS__"
    feats["char+2"] = ipa_chars[idx + 2] if idx < n - 2 else "__EOS__"

    feats["bigram-1"] = (feats["char-1"] + ch) if idx > 0 else "__BOS__" + ch
    feats["bigram+1"] = (ch + feats["char+1"]) if idx < n - 1 else ch + "__EOS__"
    if idx > 0 and idx < n - 1:
        feats["trigram"] = ipa_chars[idx - 1] + ch + ipa_chars[idx + 1]
    else:
...
def _extract_ipa_word_features(ipa_chars: list[str]) -> list[dict[str, str | float]]:
    """Extrait les features pour tous les phonemes IPA."""
...
# ══════════════════════════════════════════════════════════════════════════════
# Backend CRF (Viterbi + beam Viterbi pour top-K)
# ══════════════════════════════════════════════════════════════════════════════

class CrfP2gModel:
    """Modele CRF P2G avec decodage Viterbi pur Python."""
    ...
    @classmethod
    def load(cls, path: Path | str) -> CrfP2gModel:
    ...
    def _viterbi(self, observations: list[dict[str, str | float]]) -> list[str]:
        """Decodage Viterbi standard (top-1)."""
        n = len(observations)
        if n == 0:
            return []
    ...
    def _viterbi_topk(
        ...
        """Beam Viterbi : retourne les K meilleures sequences avec log-probs."""
        n = len(observations)
        if n == 0:
            return [([], 0.0)]
        ...
        for t in range(1, n):
            ...
            for score_prev, path in beams:
                prev_tag = tags[path[-1]]
                for j, tag_j in enumerate(tags):
                    state_score = self._score_state(observations[t], tag_j)
                    trans = self.transitions.get(prev_tag, {}).get(tag_j, 0.0)
                    new_score = score_prev + trans + state_score
        ...
        for score, path in beams:
            tag_seq = [tags[idx] for idx in path]
            results.append((tag_seq, score))

        return results

    def predict(self, ipa: str) -> str:
        """Predit l'orthographe pour une chaine IPA."""
        if not ipa:
            return ""
        ipa_chars = iter_phonemes(ipa)
        features = _extract_ipa_word_features(ipa_chars)
    ...
    def predict_candidates(
        ...
        ipa_chars = iter_phonemes(ipa)
        features = _extract_ipa_word_features(ipa_chars)
        topk = self._viterbi_topk(features, k=k * 2)

        results: list[tuple[str, float]] = []
        seen: set[str] = set()
        for tag_seq, score in topk:
            word = _reconstruct_ortho(tag_seq)
            if word not in seen:
                seen.add(word)
                seq_len = max(len(tag_seq), 1)
                results.append((word, score / seq_len))
        ...
        if results:
            ...
            results = [(w, e / total) for (w, _), e in zip(results, exp_scores)]

        return results


# ══════════════════════════════════════════════════════════════════════════════
# Backend BiLSTM (ONNX Runtime)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Backend Seq2Seq (encoder-decoder ONNX + beam search)
# ══════════════════════════════════════════════════════════════════════════════
...
def _auto_detect_backend(model_path: Path | str) -> str:
    """Detecte le type de backend depuis un chemin de fichier.

    .json                    → crf
    .onnx (sans decoder)     → bilstm
    .onnx (avec decoder)     → seq2seq
    """
    p = Path(model_path)
    if p.suffix == ".json":
        return "crf"
    if p.suffix == ".onnx":
        ...
        for f in parent.iterdir():
            if "decoder" in f.name.lower() and f.suffix == ".onnx":
                return "seq2seq"
        return "bilstm"
    return "crf"


def _find_seq2seq_files(model_dir: Path) -> tuple[Path | None, Path | None, Path | None]:
...
# ══════════════════════════════════════════════════════════════════════════════
# Classe principale LecturaP2G (facade multi-backend)
# ══════════════════════════════════════════════════════════════════════════════

class LecturaP2G:
    """Convertisseur Phoneme → Grapheme pour le francais.

    Facade multi-backend supportant CRF, BiLSTM et Seq2Seq.

    Strategies (par ordre de priorite pour predict()) :
    1. Modele ML (CRF, BiLSTM ou Seq2Seq selon backend)
    ...
    Args:
        model_path: Chemin vers le fichier modele (JSON ou ONNX encoder).
        decoder_path: Chemin vers le decoder ONNX (Seq2Seq, auto-detecte si absent).
        vocab_path: Chemin vers le vocabulaire JSON (BiLSTM/Seq2Seq, auto-detecte).
    ...
    def __init__(
        ...
        self._backend = "table+regles"

        if model_path is not None:
            model_path = Path(model_path)
            if model_path.exists():
                backend_type = _auto_detect_backend(model_path)

                if backend_type == "crf":
                    self._inner = CrfP2gModel.load(model_path)
                    self._backend = "crf"

                elif backend_type == "bilstm":
                    if vocab_path is None:
                        vocab_path = model_path.parent / "p2g_vocab.json"
                    vocab_path = Path(vocab_path)
                    if vocab_path.exists():
                        self._inner = OnnxP2gBiLSTM(model_path, vocab_path)
                        self._backend = "bilstm"

                elif backend_type == "seq2seq":
                    parent = model_path.parent
                    if decoder_path is None:
                        for f in sorted(parent.iterdir()):
                            name = f.name.lower()
                            if "decoder" in name and name.endswith(".onnx"):
                                decoder_path = f
                                break
                    if vocab_path is None:
                        for f in sorted(parent.iterdir()):
                            name = f.name.lower()
                            if name.endswith("_vocab.json"):
                                vocab_path = f
                                break
                    if decoder_path and vocab_path:
                        self._inner = OnnxP2gSeq2Seq(
                            model_path, decoder_path, vocab_path,
                        )
    ...
    def backend(self) -> str:
        """Retourne le type de backend actif : 'crf', 'bilstm', 'seq2seq' ou 'table+regles'."""
        return self._backend

    @property
    def has_model(self) -> bool:
        """True si un modele ML est charge."""
        return self._inner is not None

    def predict(self, ipa: str) -> str:
        """Meilleure orthographe pour une chaine IPA.

        Utilise le modele ML si disponible, sinon table, sinon regles.
        """
        if not ipa:
            return ""

        if self._inner is not None:
            if self._backend == "seq2seq":
                return self._inner.predict_greedy(ipa)
            else:
                return self._inner.predict(ipa)
    ...
    def predict_candidates(
        ...
        if self._inner is not None:
            if self._backend == "seq2seq":
                return self._inner.predict_beam(ipa, beam_width=k)
            else:
                return self._inner.predict_candidates(ipa, k=k)
...
# ══════════════════════════════════════════════════════════════════════════════
# Chargement par backend (pour CLI --compare et sous-dossiers)
# ══════════════════════════════════════════════════════════════════════════════

def _load_backend(here: Path, backend: str) -> LecturaP2G:
    """Charge un backend P2G depuis son sous-dossier standard."""
    if backend == "crf":
        model_path = here / "P2G_CRF" / "modele" / "p2g_model_crf.json"
        if model_path.exists():
            return LecturaP2G(model_path)
        return LecturaP2G()

    elif backend == "bilstm":
        model_path = here / "P2G_BiLSTM" / "modele" / "p2g_bilstm_int8.onnx"
        vocab_path = here / "P2G_BiLSTM" / "modele" / "p2g_vocab.json"
        if model_path.exists() and vocab_path.exists():
            return LecturaP2G(model_path, vocab_path=vocab_path)
        return LecturaP2G()

    elif backend == "seq2seq":
        model_dir = here / "P2G_Seq2Seq" / "modele"
        enc_path, dec_path, vocab_path = _find_seq2seq_files(model_dir)
        if enc_path and dec_path and vocab_path:
            return LecturaP2G(enc_path, decoder_path=dec_path, vocab_path=vocab_path)
        return LecturaP2G()
...
def main() -> None:
    ...
    if not ipa_args:
        print(f"Lectura P2G v{__version__} — facade multi-backend")
        print()
        print("Usage : python lectura_p2g.py [--crf|--bilstm|--seq2seq|--compare] ipa1 ipa2 ...")
        print()
        ...
        print()
        print("Exemple (table + regles) :")
        p2g = LecturaP2G()
        for ipa in ["bɔ̃ʒuʁ", "mɛzɔ̃", "ʃa", "o", "pɛʃœʁ", "ɑ̃fɑ̃"]:
            print(f"  /{ipa}/ → {p2g.predict(ipa)}")
        return
    ...
    if chosen == "compare":
        ...
        if chosen is None:
            chosen = "crf"
        try:
            p2g = _load_backend(here, chosen)
        except Exception as e:
            print(f"ERREUR : impossible de charger le backend {chosen}: {e}",
                  file=sys.stderr)
            p2g = LecturaP2G()

        print(f"Lectura P2G v{__version__} (backend: {p2g.backend}, "
              f"modele: {'oui' if p2g.has_model else 'non'})")
