#!/usr/bin/env python3
"""Entraîne un modèle BiLSTM morphologique et l'exporte en ONNX — Script autonome.

Architecture : char-CNN (96d) + word embedding (192d) → BiLSTM 2 couches (192h)
               → émissions (~200 tags composites POS+morpho).
...
Usage :
    python entrainer_bilstm.py \\
        --corpus ../../../POS/POS_CRF/entrainement/donnees/pos_train_merged.conllu \\
        --dev ../../../POS/POS_CRF/entrainement/donnees/pos_dev_merged.conllu
...
# Traits morphologiques clés à extraire
_MORPHO_KEYS = ("Gender", "Number", "Tense", "Mood", "Person", "VerbForm")

# Mapping des valeurs UD vers nos abréviations
...
def _build_composite_tag(base_pos: str, feats: dict[str, str]) -> str:
    ...
    core_pos = base_pos.split(":")[0]  # VER, AUX, NOM, ADJ, etc.
...
def convert_sentences(
    ...
    """
    # First pass: compute tag counts if not provided
    ...
    # Build set of valid tags (above min_tag_count)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Entraîne un BiLSTM morphologique → ONNX")
    parser.add_argument("--corpus", type=Path, default=default_corpus)
    parser.add_argument("--dev", type=Path, default=default_dev)
    parser.add_argument("--output", default=str(here / ".." / "modele" / "morpho_model_bilstm.onnx"))
    parser.add_argument("--vocab-output", default=str(here / ".." / "modele" / "morpho_vocab_bilstm.json"))
    parser.add_argument("--word-embed-dim", type=int, default=192)
    parser.add_argument("--char-embed-dim", type=int, default=32)
    parser.add_argument("--char-cnn-dim", type=int, default=96)
    ...
    parser.add_argument("--class-weights", action="store_true",
                        help="Utiliser la pondération inverse des fréquences")
    ...
    train_data, tag_counts = convert_sentences(raw_train, min_tag_count=args.min_tag_count)
    ...
    dev_data: list[tuple[list[str], list[str], list[str]]] = []
    if args.dev.exists():
        raw_dev = parse_conllu(str(args.dev))
        dev_data, _ = convert_sentences(raw_dev, min_tag_count=args.min_tag_count,
    ...
    if args.class_weights:
        total_tokens = sum(len(tags) for _, tags, _ in train_data)
        weight_list = []
        for i in range(n_tags):
            tag = idx2tag[i]
            count = sum(1 for _, tags, _ in train_data for t in tags if t == tag)
            freq = count / total_tokens if total_tokens else 1
            weight_list.append(1.0 / (freq * n_tags) if freq > 0 else 1.0)
    ...
    class MorphoDataset(Dataset):
        ...
        def __getitem__(self, idx):
            words, tags, _lemmas = self.data[idx]
    ...
    def evaluate_data(data, label):
        ...
        pos_correct = 0
        with torch.no_grad():
            for words, tags, _lemmas in data:
                word_ids = torch.tensor(
                    [[word2idx.get(w.lower(), 1) for w in words]], dtype=torch.long
                ).to(device)
                char_ids_list = []
                ...
                for pred_idx, gold_tag in zip(pred_ids[: len(tags)], tags):
                    ...
                    if pred_tag == gold_tag:
                        correct += 1
                    # POS-only accuracy
                    pred_pos = pred_tag.split("|")[0]
                    gold_pos = gold_tag.split("|")[0]
                    if pred_pos == gold_pos:
                        pos_correct += 1

        acc = correct / total if total else 0
        pos_acc = pos_correct / total if total else 0
        print(f"  {label} Accuracy complète : {acc:.1%} ({correct}/{total})")
        print(f"  {label} Accuracy POS seul : {pos_acc:.1%} ({pos_correct}/{total})")
