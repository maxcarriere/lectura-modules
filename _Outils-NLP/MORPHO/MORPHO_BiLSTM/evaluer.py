#!/usr/bin/env python3
"""Évaluation de l'analyseur morphologique Lectura (BiLSTM) sur corpus UD.
...
def _build_composite_tag(base_pos: str, feats: dict[str, str]) -> str:
    """Même logique que dans entrainer_bilstm.py."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Évaluation de l'analyseur morphologique BiLSTM Lectura"
    )
    parser.add_argument(
        "--test", required=True,
    ...
    parser.add_argument(
        "--model", default=str(here / "modele" / "morpho_model_bilstm_int8.onnx"),
        help="Chemin du modèle ONNX",
    )
    parser.add_argument(
        "--vocab", default=str(here / "modele" / "morpho_vocab_bilstm.json"),
    ...
    # Charger le vocabulaire pour connaître les tags valides
    vocab_path = Path(args.vocab)
    valid_tags = set()
    if vocab_path.exists():
        with open(vocab_path, encoding="utf-8") as f:
            vocab_data = json.load(f)
        valid_tags = set(vocab_data.get("tags", []))
    ...
    print("\nChargement du modèle BiLSTM morpho...")
    tagger_base = MorphoTagger(args.model, vocab_path=args.vocab)
    ...
    print_results("BiLSTM morpho (sans GLAFF)", results_base, trait_stats_base, lemma_stats_base)

    # ── Config 2 : BiLSTM + GLAFF ──
    lexicon_path = Path(args.lexicon)
    if lexicon_path.exists():
        print("\nChargement du lexique GLAFF...")
        tagger_lex = MorphoTagger(args.model, vocab_path=args.vocab, lexicon_path=args.lexicon)
        n_entries = len(tagger_lex.lexicon)
        print(f"  {n_entries} entrées chargées")

        print("Évaluation BiLSTM + GLAFF...")
        pred_tags_lex = []
        pred_lemmas_lex = []
        for words in words_all:
        ...
        print_results("BiLSTM morpho + GLAFF", results_lex, trait_stats_lex, lemma_stats_lex)
