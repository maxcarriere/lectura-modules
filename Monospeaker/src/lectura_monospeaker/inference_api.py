"""Backend API pour le TTS — delegue l'inference au serveur Lectura.
...
class ApiTTSEngine:
    """Backend API — meme interface que OnnxTTSEngine.
    ...
    def synthesize(
        ...
        **kwargs,
        ...
        if energy_scale is not None:
            payload["energy_scale"] = energy_scale
        if pause_scale is not None:
            payload["pause_scale"] = pause_scale
        if duration_noise is not None:
            payload["duration_noise"] = duration_noise
        if style is not None:
            payload["style"] = style
        if style_vector is not None:
            payload["style_vector"] = style_vector
        if n_ode_steps is not None:
            payload["n_ode_steps"] = n_ode_steps
    ...
    def _call_api(self, payload: dict) -> TTSResult:
        ...
        try:
            ...
            # Sans numpy, retourner les bytes bruts
