"""Localisateur de modeles OpenVoice --- cascade de chemins.
...
# Fichiers requis par OpenVoice
OPENVOICE_REQUIRED = ["openvoice_se.onnx", "openvoice_vc.onnx"]


def find_models_dir(models_dir: str | Path | None = None) -> Path | None:
    """Trouve le repertoire contenant les modeles OpenVoice.
    ...
    for candidate in candidates:
        if candidate.is_dir():
            log.debug("Repertoire modeles VC-ZeroShot candidat : %s", candidate)
            return candidate

    return None


def has_openvoice_models(directory: Path) -> bool:
    """Verifie la presence des modeles OpenVoice."""
    if not directory.is_dir():
        return False

    for filename in OPENVOICE_REQUIRED:
...
def load_model_bytes(models_dir: Path, filename: str) -> bytes | None:
    ...
    if path.exists():
        ...
        from lectura_vc_zeroshot._crypto import load_encrypted_model
