"""Localisateur de modeles VC --- cascade de chemins.
...
def find_models_dir(models_dir: str | Path | None = None) -> Path | None:
    """Trouve le repertoire contenant les modeles VC.
    ...
    for candidate in candidates:
        if candidate.is_dir():
            log.debug("Repertoire modeles VC candidat : %s", candidate)
            return candidate
...
def load_model_bytes(models_dir: Path, filename: str) -> bytes | None:
    ...
    if path.exists():
        ...
        from lectura_vc._crypto import load_encrypted_model
